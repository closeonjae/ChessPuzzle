...
**`solution[]`의 첫 항목이 누구 수인지, 그리고 `initialPly`의 진짜 의미 — python-chess로 최종 검증**: `/next` 문서의 예시 퍼즐(`QBX2O`, `solution: ["f2g1","h1g1","c8c1"]`, `initialPly=66`)을 처음엔 "체크·메이트 두 수가 솔버, 강제 응수 한 수가 상대"라는 **체스 패턴의 정성적 추론만으로** `solution[0]`이 솔버의 수라고 결론 냈었다 — 어느 편이 체크를 거는지는 실제로 안 세어봤다는 뜻이라, 틀릴 수 있는 추론이었다. 이후 사용자의 실제 워치에 `adb pair`/`connect`로 직접 붙어 잡은 실제 퍼즐(`tOfGm`, `game.pgn`이 29토큰, `initialPly=28`, `solution=[d7h3, f1f2, h3h5]`)을 **python-chess로 그대로 재생**해 검증하니(도구: `pip install chess`, `board.push_san()` 반복 후 `board.legal_moves` 확인) 명확한 모순이 나왔다 — `game.pgn`의 첫 `initialPly`(28)개 토큰을 그대로 재생하면 백 차례가 되는데 `solution[0]="d7h3"`(흑 퀸)은 그 상태에서 불법수였다. `initialPly - 1`(27)개만 재생하면 흑 차례가 되고 합법수가 됐다. 같은 방식으로 `QBX2O` 예시도 재검증(66개 재생 시 불법, 65개 재생 시 합법) — 우연이 아니라 일관된 패턴이었다.

→ **1차 결론(불완전 — `solution[0]` 하나만 확인했음)**: `initialPly - 1`개만 재생하면 `solution[0]`은 합법이 됐다. 이걸 "1-indexed 다음 ply 번호"라는 결론으로 배포했는데, 실기기에서 바로 다음 크래시가 잡혔다 — `solution[1]`("f1f2")부터는 여전히 불법수였던 것. `solution[0]` 하나의 합법성은 진짜 시작 포지션을 찾았다는 증거로 **불충분**했다(우연히 legal한 다른 move일 수 있음 — 실제로 그랬다).

→ **최종 결론**: 부분 재생 개수를 맞히는 접근을 버리고, `solution` 전체가 순서대로 끝까지 합법이 되는 재생 개수를 찾았다 — 두 실제 예시(`tOfGm`, `QBX2O`) 모두 **`game.pgn`을 통째로 다 재생했을 때만** 전체 수순이 합법이었고, `initialPly`/`initialPly - 1`을 포함한 다른 어떤 부분 재생 개수도 실패했다. 즉 **`initialPly`는 재생 개수와 아예 무관한 필드**다 — `game.pgn`은 항상 "퍼즐이 시작되기 직전까지의 실제 게임"으로 끝나도록 주어지므로, 그냥 전부 재생하면 된다. `initialPly`가 정확히 무엇을 위한 필드인지(UI 표시용? 로그용?)는 여전히 불명 — API 문서에 설명이 없고, 재생 로직에는 더 이상 쓰지 않는다. 정성적 추론(솔버가 체크메이트를 건다, `solution[0]`은 솔버의 수) 자체는 결과적으로 맞았다 — REST API는 솔버가 먼저, CSV 퍼즐 데이터베이스 덤프만 상대가 먼저인 게 맞다. `PuzzleEngine.kt`의 게임 리플레이 로직을 "전체 재생"으로 단순화하고, 실제 크래시를 그대로 재현하는 회귀 테스트를 추가함(DESIGN.md 5절).
