...
→ **결론**: `initialPly`는 "재생할 ply 개수"가 아니라 **1-indexed로 "다음에 둘 ply의 번호"**다. 시작 포지션을 만들려면 `game.pgn`의 앞 `initialPly - 1`개 토큰만 재생해야 한다. 정성적 추론(솔버가 체크메이트를 건다)은 결과적으로 맞았다 — `solution[0]`은 여전히 솔버의 수다(REST API는 솔버가 먼저, CSV 퍼즐 데이터베이스 덤프만 상대가 먼저인 게 맞음) — 하지만 그 확인 과정 자체가 부정확했고, 실제로 앱을 계속 깨뜨린 원인은 "누가 먼저 두느냐"가 아니라 이 순수한 **off-by-one**이었다. `PuzzleEngine.kt`의 게임 리플레이 로직을 `initialPly - 1`로 수정하고, 실제 `tOfGm` 데이터를 그대로 넣은 회귀 테스트를 추가함(DESIGN.md 5절).
